...
export function UserDetailDrawer({ user, isOpen, onClose, onSendNotification }: UserDetailDrawerProps) {
  ...
  // Return null if no user selected or drawer not open
  if (!isOpen || !user) {
    return null;
  }
  ...
  return (
    <>
      ...
      <div className={`fixed right-0 top-0 h-full w-full max-w-2xl bg-card border-l border-card-border shadow-lg transition-transform z-50 overflow-y-auto translate-x-0`}>
        {/* Header */}
        ...
        <div className="p-6 space-y-6">
            ...
            {activeTab === "devices" && (
              <div className="space-y-2">
                {devicesLoading ? (
                  ...
                  (devices || []).map((device) => (
                    <div key={device?.id || Math.random()} className="p-3 bg-muted/10 rounded-lg border border-muted/20">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Smartphone className="w-4 h-4 text-muted-foreground" />
                          <div>
                            <p className="text-sm font-medium">{device?.name || "Unknown Device"}</p>
                            <p className="text-xs text-muted-foreground">{device?.browser || "Unknown"} • {device?.os || "Unknown"}</p>
                          </div>
                        </div>
                        <p className="text-xs text-muted-foreground">{device?.lastActive || "Unknown"}</p>
                      </div>
                    </div>
                  ))
                )}
              ...
              <div className="space-y-2">
                {historyLoading ? (
                  ...
                  (loginHistory || []).map((login) => (
                    <div key={login?.id || Math.random()} className="p-3 bg-muted/10 rounded-lg border border-muted/20">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Globe className="w-4 h-4 text-muted-foreground" />
                          <div>
                            <p className="text-sm font-medium">{login?.browser || "Unknown"} • {login?.device || "Unknown"}</p>
                            <p className="text-xs text-muted-foreground">{login?.date || "Unknown date"}</p>
            ...
            {activeTab === "plans" && (
              <div className="space-y-2">
                {plansLoading ? (
                  ...
                  (plans || []).map((plan) => (
                    <div key={plan?.id || Math.random()} className="p-3 bg-muted/10 rounded-lg border border-muted/20">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-sm font-medium">{plan?.name || "Unknown Plan"}</p>
                        <Badge className={(plan?.status === "active") ? "bg-green-500/20 text-green-400" : "bg-gray-500/20 text-gray-400"}>
                          {plan?.status || "unknown"}
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <span className="capitalize">{plan?.network || "unknown"}</span>
                        <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {plan?.expiryDate || "Unknown"}</span>
                      </div>
                    </div>
                  ))
                )}
          ...
          <div className="flex gap-2 pt-4 border-t border-card-border">
            <Button
              ...
              disabled={!user?.id}
              className="flex-1 bg-primary/20 hover:bg-primary/30 text-primary gap-2 disabled:opacity-50"
            >
